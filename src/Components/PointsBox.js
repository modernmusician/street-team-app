...
function PointsBox({
  ...
  if (includeText) {
    if (totalPoints === 0) {
      pointsInfoText =
        'Register now and start earning points towards your entry.';
    } else if (totalPoints > 0 && totalPoints < 500) {
      pointsInfoText = (
        <Text color="white">
          You're already in the{' '}
          <Text color="white" weight="642" opacity="1">
            {contestRanking}
          </Text>{' '}
          of possible winners!
        </Text>
  ...
  if (totalReferrals > 0 && bonusPoints > 0) {
    totalReferralsText =
      totalReferrals + ' referrals, ' + bonusPoints + ' bonus points';
  }
  ...
  return (
    <div className="points-container">
      ...
      <div className="points-info-box">
        ...
        <p className="points-info-text-referrals fade-light">
          {' '}
          {totalReferralsText}{' '}
        </p>
