...
const summaryOfEnduser = (input) => {
    ...
    const output = input?.enduserPageSubscriptionCompletedActions?.items.forEach(action => {
...
export const Demo = () => {
    const {
        ...
        loading,
        error,
    } = useQuery(gql(getAllSubscribersFromArtistUser),{
    ...
    if(subscriberData){
        ...
        const summaryData = subscriberData.getArtistUser.artist.actionPages.items[0].subscribers.items.map(item => summaryOfEnduser(item));
